    ...
    <Link href={`/campaigns/${id}`} className="block h-full group">
      <Card className="relative h-full overflow-hidden rounded-3xl border-none bg-white p-6 shadow-soft-md transition-all duration-300 hover:-translate-y-1 hover:shadow-soft-xl">
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-xl font_-bold text-slate-900 group-hover:text-slate-700 transition-colors">{title}</h3>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm font_-medium">
              ...
              <span className="text-slate-900 font_-bold">{formattedRaised}</span>
              <span className="text-slate-400">of {formattedGoal}</span>
            </div>
            <Progress value={percentage} className="h-3 rounded-full bg-slate-100" />
          </div>

          <Button className="w-full rounded-xl bg-slate-900 text-white shadow-lg transition-all duration-500 hover:bg-[#fbe72b] hover:text-slate-900 hover:scale-[1.02] py-6 text-base font_-bold">
            Donate Now
          </Button>
        </div>
      </Card>
