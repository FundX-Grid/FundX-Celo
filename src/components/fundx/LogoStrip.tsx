...
const LogoItem = ({ name, hoverColor = '#fbe72b', fontSize = '3xl', fontWeight = 'font-bold' }) => {
  return (
    <div className="flex items-center justify-center h-12 cursor-default group">
      <span className={`text-${fontSize} ${fontWeight} tracking-tight text-slate-300 transition-colors duration-300 group-hover:text-${hoverColor}`}>{name}</span>
    </div>
  )
}
