...
const LogoItem = ({ name, textSize = '3xl', textColor = '#fbe72b' }) => {
  return (
    <div className="flex items-center justify-center h-12 cursor-default group">
      <span className={`text-${textSize} font-bold tracking-tight text-slate-300 transition-colors duration-300 group-hover:text-[${textColor}]`}>{name}</span>
    </div>
  )
}
