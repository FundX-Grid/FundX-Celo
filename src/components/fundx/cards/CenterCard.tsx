...
const renderContentSide = (campaign: Campaign, progress: number) => (
  <div className="flex flex-col justify-between p-8 md:w-7/12 h-full bg-white">
    ...
    <div className="space-y-6">
      ...
      {renderRaisedAndButton(campaign)}
    </div>
  </div>
);
...
const renderRaisedAndButton = (campaign: Campaign) => (
  <div className="flex justify-between items-end">
    <div>
      <p className="text-xs text-slate-400 uppercase font-semibold mb-1">Raised</p>
      <p className="text-3xl font-bold text-primary">${campaign.raised.toLocaleString()}</p>
    </div>
...
export function CenterCard({ campaign, progress }: CenterCardProps) {
  return (
    <div className="w-full h-full relative z-20 shadow-2xl rounded-[2rem] border border-slate-100 bg-white hover:border-orange-200 transition-all duration-300 flex flex-col md:flex-row overflow-hidden group">
      {renderTrendingBadge()}
      {renderImageSide()}
      {renderContentSide(campaign, progress)}
    </div>
  );
}
