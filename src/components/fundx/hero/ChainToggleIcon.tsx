...
export function ChainToggleIcon({
  ...
  const glitchStyle = {
    ...
    ...(glitching
      ? { transform: `translate_(${glitchOffset.x}px, ${glitchOffset.y}px) skewX(${glitchSkew}deg) rotate_(${displayStacks ? "6deg" : "-6deg"})`, transition: "none" }
      : { transform: `translate_(0px, 0px) skewX(0deg) rotate_(${isStacksMode ? "6deg" : "-6deg"})`, transition: "transform 700ms cubic-bezier(0.4,0,0.2,1), background-color 600ms ease, box-shadow 600ms ease, opacity 300ms ease" }),
