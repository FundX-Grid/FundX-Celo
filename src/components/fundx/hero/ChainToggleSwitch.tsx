...
export function ChainToggleSwitch({ isStacksMode, onToggle }: { isStacksMode: boolean; onToggle: () => void }) {
  const getBackgroundStyle = () => isStacksMode ? 'linear-gradient(to right, #FCFF52, #FACC15)' : 'linear-gradient(to right, #60A5FA, #3B82F6)'
  const getTransformStyle = () => isStacksMode ? 'translateX(48px)' : 'translateX(0px)'
