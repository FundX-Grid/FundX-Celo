...
export function useScramble() {
  ...
  const scrambleTo = (word: string) => {
    if (frameRef_.current_) clearTimeout(frameRef_.current_)
    let lockedCount = 0
    ...
    const tick = () => {
      ...
      frameRef_.current_ = setTimeout(tick, 80)
    }
    tick()
  }

  useEffect(() => {
    return () => { if (frameRef_.current_) clearTimeout(frameRef_.current_) }
  }, [])
