...
const getAvatarBadgeClassName = (size: "default" | "sm" | "lg", className?: string) => {
  return cn(
    ...
    { "group-data-[size=sm]/avatar:[&>svg]:hidden": size === "sm" },
    { "group-data-[size=default]/avatar:size-2.5": size === "default" },
    { "group-data-[size=default]/avatar:[&>svg]:size-2": size === "default" },
    { "group-data-[size=lg]/avatar:size-3": size === "lg" },
    { "group-data-[size=lg]/avatar:[&>svg]:size-2": size === "lg" },
    className
  )
}
...
const getAvatarGroupCountClassName = (size: "default" | "sm" | "lg", className?: string) => {
  return cn(
    ...
    { "[&>svg]:size-4": size === "default" },
    { "group-has-data-[size=lg]/avatar-group:[&>svg]:size-5": size === "lg" },
    { "group-has-data-[size=sm]/avatar-group:[&>svg]:size-3": size === "sm" },
    className
  )
}
