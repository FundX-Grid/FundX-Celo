...
const getAvatarClassName = (size: "default" | "sm" | "lg", className?: string) => {
  const baseClass = "group/avatar relative flex size-8 shrink-0 overflow-hidden rounded-full select-none"
  const sizeClass = {
    default: "",
    sm: "data-[size=sm]:size-6",
    lg: "data-[size=lg]:size-10",
  }[size]
  return cn(baseClass, sizeClass, className)
}
...
const getAvatarFallbackClassName = (size: "default" | "sm" | "lg", className?: string) => {
  const baseClass = "bg-muted text-muted-foreground flex size-full items-center justify-center rounded-full text-sm"
  const sizeClass = {
...
const getAvatarBadgeClassName = (size: "default" | "sm" | "lg", className?: string) => {
  const baseClass = "bg-primary text-primary-foreground ring-background absolute right-0 bottom-0 z-10 inline-flex items-center justify-center rounded-full ring-2 select-none"
  const sizeClass = {
...
const getAvatarGroupCountClassName = (size: "default" | "sm" | "lg", className?: string) => {
  const baseClass = "bg-muted text-muted-foreground ring-background relative flex size-8 shrink-0 items-center justify-center rounded-full text-sm ring-2"
  const sizeClass = {
    default: "",
