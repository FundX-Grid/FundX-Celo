...
const getInputClassName = (className: string, type: string, isValid: boolean, isFocused: boolean, isDisabled: boolean) => {
  return cn(
    ...
    isDisabled ? "disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50" : "",
    className
  )
}
...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  ...
  const isFocused = document.activeElement === props.ref
  ...
  return (
    <input
      type={type}
      data-slot="input"
      className={getInputClassName(className, type, isValid, isFocused, isDisabled)}
      {...props}
    />
  )
}
