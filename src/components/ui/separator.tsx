...
function getSeparatorClassName(orientation: string, className?: string) {
  if (orientation === "horizontal") return cn("bg-border shrink-0", getHorizontalClassName(className))
  if (orientation === "vertical") return cn("bg-border shrink-0", getVerticalClassName(className))
  return ""
}
...
function Separator({ className, orientation = "horizontal", decorative = true, ...props }: React.ComponentProps<typeof SeparatorPrimitive.Root>) {
  return (
    <SeparatorPrimitive.Root data-slot="separator" decorative={decorative} orientation={orientation} className={getSeparatorClassName(orientation, className)} {...props} />
  )}
export { Separator }