...
function getCurrency(token: string) {
  return token.toLowerCase() === TOKEN_ADDRESSES.cUSD.toLowerCase() ? 'cUSD' : 'USDC';
}

function getDecimals(currency: 'cUSD' | 'USDC') {
  return currency === 'cUSD' ? TOKEN_DECIMALS.cUSD : TOKEN_DECIMALS.USDC;
}

function calculateDaysLeft(deadline: number, nowSec: number) {
  return deadline < nowSec ? 0 : Math.ceil((deadline - nowSec) / 86400);
}
...
function mapContractCampaign(raw: any, index: number): OnChainCampaign {
  ...
  const daysLeft = calculateDaysLeft(deadline, nowSec);
